...
namespace TilesEditor
{
    ...
    bool Level::loadFile(ResourceManager& resourceManager)
    {
        ...
        else if (m_fileName.endsWith(".graal") || m_fileName.endsWith(".zelda") || m_fileName.endsWith(".editor"))
            m_loadFail = !loadGraalFile(resourceManager);
    ...
    bool Level::loadGraalFile(ResourceManager& resourceManager)
    {
        ...
        {
            int v = -1;
            QString fileVersion = file.read(8);
            ...
            // Load tiles.
            {
                ...
                // Read the tiles.
                while (boardIndex < 64 * 64 && !file.atEnd())
                {
            ...
            // Load the baddies.
            {
                while (!file.atEnd())
                {
                    signed char x = file.read(1).data()[0];
                    signed char y = file.read(1).data()[0];
                    signed char type = file.read(1).data()[0];
    ...
    bool Level::saveFile()
    {
        ...
        if (m_fileName.endsWith(".graal") || m_fileName.endsWith(".zelda"))
            return saveGraalFile();
    ...
    bool Level::saveGraalFile()
    {
        QFile file(m_fileName);
        if (file.open(QIODevice::WriteOnly))
        {
            file.write("GR-V1.03", 8);
            ...
            if (tilemap)
            {
                ...
                //links
                {
                    for (auto link : m_links)
                    {
                        ...
                        file.write(linkString.toLocal8Bit().data());
                    }
                    file.write("#\n");
                }
                ...
                //baddies
                {
                    for (auto obj : m_objects)
                    {
                        if (obj->getEntityType() == LevelEntityType::ENTITY_BADDY)
                        {
                            ...
                            file.write(verses.toLocal8Bit());
                            file.write("\n");
                        }
                    }
                    char negative = -1;
                    file.write(&negative, 1);
                    file.write(&negative, 1);
                    file.write(&negative, 1);
                    file.write("\n", 1);;
                }
