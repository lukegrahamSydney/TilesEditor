...
static int vm_getprop_builtin( SGS_CTX, sgs_Variable* outmaybe, sgs_Variable* obj, sgs_Variable* idx )
{
	if( idx->type == SGS_VT_STRING )
	{
		...
		case SGS_VT_THREAD:
			if (!strcmp(prop, "vars"))
			{
				if (!sgs_IsDict(C->user_vars))
				{
					sgs_Release(C, &C->user_vars);
					sgs_CreateDict(C, &C->user_vars, 0);
				}
				*outmaybe = C->user_vars;
				sgs_Acquire(C, outmaybe);
				return 0;
			}
	...
	sgs_Variable* outmaybe, sgs_Variable* obj, sgs_Variable* idx, int isprop )
{
	if( type == SGS_ENOTFND )
	{
		...
		if( obj->type == SGS_VT_OBJECT)
		{
			...
			if (obj->data.O->iface == sgsstd_map_iface)
			{
				outmaybe->type = SGS_VT_INT;
				outmaybe->data.I = 0;
				return 0;
...
div0err:
	//VAR_RELEASE( out );
	sgs_Msg( C, SGS_WARNING, "Division by 0" );
	var_setint(C, out, 0);
	return SGS_TRUE;
fail:
	VAR_RELEASE( out );
...
static void vm_make_class( SGS_CTX, int outpos, sgs_Variable* name, sgs_Variable* inhname )
{
	...
	if( inhname )
	{
		...
		if( sgs_GetGlobal( C, *inhname, &inhcls ) && inhcls.type == SGS_VT_OBJECT )
		{
			...
			//Release should be called here. "sgs_GetGlobal" will acquire the inherited class. "sgs_ObjSetMetaObj" will acquire it a second time
			//This release corresponds to the "sgs_GetGlobal" call
			sgs_ObjRelease(C, inhcls.data.O);
...
static void vm_make_new( SGS_CTX, int outcls, int lastarg )
{
	...
	if( clscopy.type == SGS_VT_FUNC || clscopy.type == SGS_VT_CFUNC || clscopy.type == SGS_VT_DFUNC )
	{
		...
		int argstart = outcls + 1;
		int argend = lastarg + 1;
		int i;
		fstk_push(C, &clscopy);
		stk_makespace(C, argend - argstart);
		for (i = argstart; i < argend; ++i)
		...
		sgs_Call(C, argend - argstart, 1);
		sgs_StoreVariable(C, &inst);
		stk_setvar_leave(C, outcls, &inst);
...
SGS_APIFUNC SGSBOOL sgs_GetProperty(SGS_CTX, sgs_Variable obj, const char* name, sgs_Variable* out)
{
	int ret;
	sgs_PushString(C, name);
	ret = sgs_GetIndex(C, obj, *stk_gettop(C), out, SGS_TRUE);
	fstk_pop1(C);
	return ret;
}
