...
SFTRET parse_join(SFTC)
{
	...
	if (p)
	{
		if (p->token[0] == SGS_ST_STRING)
		{
			...
			int32_t nameLen;
			SGS_ST_READINT(nameLen, p->token + 1);
			nameLen = nameLen < sizeof(fname) - 1 ? nameLen : sizeof(fname) - 1;

			//get the file name
			strncpy(fname, (const char*)(p->token) + 5, nameLen);
			fname[nameLen] = 0;

			//Check if it already has been joined
			for (size_t i = 0U; i < C->joinFiles.size;)
			{
				const char* name = C->joinFiles.ptr + i;
			...
			//Add it to the joins
			sgs_membuf_appbuf(&C->joinFiles, C, fname, nameLen + 1);
			...
			FTComp F2;
			{
				F2.C = C;
				F2.at = tlist;
				F2.heap = F2.heapend = F->heapend;
			};

			ftree = parse_stmtlist(&F2, 0);
			...
			sgs_JoinedTokenList* tklist = sgs_Alloc(sgs_JoinedTokenList);
			tklist->tokens = tlist;
			tklist->next = NULL;

			if (!C->jt_first)
				C->jt_first = C->jt_last = tklist;
			else {
				C->jt_last->next = tklist;
				C->jt_last = tklist;
			}
			return ftree;
		}
...
SFTRET parse_stmt( SFTC )
{
	...
	//else if( SFTC_ISKEY( "switch" )) { return parse_switch(F); }
...
#define NOT_FCALL ( !sgsT_Next( F->at ) || '(' != *sgsT_Next( F->at ) )
	...
	/* join */
	else if (SFTC_IS_ID("join") && NOT_FCALL)
	{
		return parse_join(F);
	}
	/* BLOCK OF STATEMENTS */
