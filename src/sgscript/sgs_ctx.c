...
SGSRESULT sgs_PushSGSFunctionBuf( SGS_CTX, const char* buf, size_t size )
{
	...
	else
	{
		...
		if (C->filename != NULL)
			sgs_membuf_appbuf(&C->joinFiles, C, C->filename, strlen(C->filename) + 1);
		...
		//cleanup joined tokens
		if (C->jt_first)
		{
			for (sgs_JoinedTokenList* tklist = C->jt_first; tklist;) {
				sgs_JoinedTokenList* next = tklist->next;
				sgsT_Free(C, tklist->tokens);
				sgs_Free(C, tklist);
				tklist = next;
			}
