...
{
	m_tab = std::max(0, m_tab - 1);
}

std::string GS1Converter::getTab() const
{
	return std::string(m_tab * 4, ' ');
}
...
bool GS1Converter::convert(const std::string& input, bool clientSide)
{
	...
	auto observer = [](const gs1::Diag& d, void* userPointer) ->void
		{
			auto self = static_cast<GS1Converter*>(userPointer);

			self->setLastError(std::format("[line {0}] {1}", d.pos.line, d.message));
	...
	{
		increaseTab();
		auto rootBlock = static_cast<gs1::StmtBlock*>(rootNode.get());
		for (auto i = 0U; i < rootBlock->statements.size(); ++i)
		{
			auto child = rootBlock->statements[i];
			child->Accept(this);
		...
		for (auto& it : m_actionCode)
		{
			...
			auto actionCode = it.second.first.str();
			...
			if (it2 != validActions.end())
			{
				...
				if (m_createCode.str().size() && actionName == "created")
				{
					m_output << m_createCode.str() << "\n";
					m_createCode = std::stringstream();
				}
				...
				if (m_addParseGS1ToAllActions && !hasParseGS1Call)
				{
					...
					std::string contextualPlayer = "player";
					...
					if (it != validActions.end())
					{
						...
						for (auto i = 1U; i < funcHeader.size(); ++i)
						{
							...
							{
								contextualNPC = "npc";
							} else if (funcHeader[i] == "P")
							{
								contextualPlayer = "player";
							}
						}

						m_output << contextualPlayer << ", " << contextualNPC;
						m_output << ");\n";
					}
				}
		...
		if (m_parseActionFunction.str().size() > 0)
		{
			m_output << "function parseGS1Action(actionName, player, npc)\n";
			m_output << "{\n";
			m_output << m_parseActionFunction.str();
			m_output << "}\n";
		}

		if (m_generatedFunctionsCode.str().size() > 0)
		{
			m_output << m_generatedFunctionsCode.str();
		}
		return true;
	}
	return false;
...
bool GS1Converter::testCodeForErrors(const std::string& code, std::string* errorOutput)
{
	auto observer = [](const gs1::Diag& d, void* userPointer) ->void
		{
			auto self = static_cast<GS1Converter*>(userPointer);
...
static inline size_t GetTrimmedRightPos(const std::string& input)
{
	size_t right = input.length();
	for (auto it = input.rbegin(); it != input.rend(); ++it)
	{
		if (!std::isspace(input[right - 1]))
			break;
	}
	return right;
}
...
/*
template <class T>
static size_t Split(const std::string& str, const std::string& del, T inserter)
{
	auto start = 0U;
	...
	while (end != std::string::npos)
	{
		...
		auto len = GetTrimmedRightPos(token);

		if (len > 0)
		{
			*inserter++ = token.substr(0, len);
			++count;
		}
	...
	auto token = str.substr(start, end);
	auto len = GetTrimmedRightPos(token);
	if (len > 0)
	{
		*inserter++ = str.substr(start, end);
		return ++count;
	}
	return count;
}
...
template <class T>
static size_t Split(const std::string& s, const std::string& sep, T inserter)
{
	...
	{
		*inserter++ = s.substr(prev_pos, pos - prev_pos);
	...
	{
		*inserter++ = s.substr(prev_pos); // Last word
		++count;
	}
...
std::string GS1Converter::revert(const std::string& input)
{
	...
	if (pos != std::string::npos)
	{
		std::string code = input.substr(pos);
		std::list<std::string> lines;
		if (Split(code, "\n", std::back_inserter(lines)))
		{
			std::string retval = "";
...
{
	return std::string(m_output.str());
}
...
bool GS1Converter::replaceputhorse(std::vector<gs1::Expr*>& args)
{
	if (args.size() >= 3)
	{
		output() << "createObject(\"horse\", this.level, (";
		if (args[1]) args[1]->Accept(this);
		output() << ") * 16, (";
		if (args[2])args[2]->Accept(this);
		output() << ") * 16, this.tileLayer, ";
		if (args[0]) args[0]->Accept(this);
		output() << ")";
...
bool GS1Converter::replaceputcomp(std::vector<gs1::Expr*>& args)
{
	...
	if (args.size() >= 3)
	{
		...
		output() << "createObject(\"Baddy\", " << getLevelTarget() << ", (";
		if (args[1]) args[1]->Accept(this);
		output() << ") * 16, (";
		if (args[2]) args[2]->Accept(this);
		output() << ") * 16, this.tileLayer, ";
...
bool GS1Converter::replaceputnewcomp(std::vector<gs1::Expr*>& args)
{
	...
	if (args.size() >= 5)
	{
		...
		output() << "createObject(\"Baddy\", this.level, (";
		if (args[1]) args[1]->Accept(this);
		output() << ") * 16, (";
		if (args[2]) args[2]->Accept(this);
		output() << ") * 16, this.tileLayer, ";
...
bool GS1Converter::replacecarryobject(std::vector<gs1::Expr*>& args)
{
	if (args.size() >= 1)
	{
		if (args[0] && args[0]->GetType() == "ExprStringLiteral")
		{
			...
			output() << "player.carryObject = createObject(\"" + className + "\",  this, player.x, player.y, player.tileLayer)";
		}
...
bool GS1Converter::replaceputbomb(std::vector<gs1::Expr*>& args)
{
	if (args.size() >= 3)
	{
		output() << "createObject(\"bomb\", this.level, (";
		if(args[1]) args[1]->Accept(this);
		output() << ") * 16, (";
		if (args[2]) args[2]->Accept(this);
		output() << ") * 16, ";
...
bool GS1Converter::replaceputexplosion(std::vector<gs1::Expr*>& args)
{
	if (args.size() >= 3)
	{
		output() << "createExplosion(this.level, ";
		if (args[0])args[0]->Accept(this);
		output() << ", (";
		if (args[1])args[1]->Accept(this);
		output() << ") * 16, (";
...
bool GS1Converter::replacesetlevel2(std::vector<gs1::Expr*>& args)
{
	if (args.size() >= 3)
	{
		...
		if (args[1]) args[1]->Accept(this);
		output() << ") * 16, (";
		if (args[2]) if (args[2])args[2]->Accept(this);
		output() << ") * 16";
		output() << ")";
		return true;
	}
...
std::string GS1Converter::getLevelTarget() const
{
	if (m_useLocalCoordinates)
		return "this.level";
	return "this";
...
void GS1Converter::Visit(gs1::StmtBlock* node)
{
	...
	for (auto i = 0; i < node->statements.size(); ++i)
	{
		auto child = node->statements[i];
	...
	if (m_blockLevel == 0 && node->elseBody == nullptr)
	{
		if (node->cond->GetType() == "ExprId")
		{
			...
			if (validActions.find(actionName) != validActions.end())
			{
				...
				if (node->thenBody && node->thenBody->GetType() == "StmtBlock")
				{
					auto thenBlock = static_cast<gs1::StmtBlock*>(node->thenBody);
					for (auto i = 0; i < thenBlock->statements.size(); ++i)
					{
						auto child = thenBlock->statements[i];
...
removeActions:
	...
	if (m_blockLevel == 0)
	{
		...
		else if (m_tempActionsList.size() == 0)
		{
			...
			m_parseActionFunction << tempStream.str() << "\n";
		...
		for (auto action : m_tempActionsList)
		{
			...
			if (!thing.second)
			{
				...
				std::stringstream stream = std::stringstream() << getTab() << "this.parseGS1Action(\"";
				stream << action << "\", ";
				...
				if (it != validActions.end())
				{
					...
					stream << contextualPlayer << ", " << contextualNPC;
					stream << ");\n";
				}
				m_actionCode[action].first = std::stringstream() << m_actionCode[action].first.str() << stream.str();
...
void GS1Converter::Visit(gs1::StmtWith* node)
{
	output() << "{\n";
	if (node->init)
	{
...
void GS1Converter::Visit(gs1::ExprCall* node)
{
	if (node->left->GetType() == "ExprId")
	{
		...
		if (!commandReplacement(identifier, node->args))
		{
			...
			for (auto it : gs1::prototypes_funcs)
			{
