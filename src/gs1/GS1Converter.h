...
private:
	...
	std::unordered_map<std::string, std::pair<std::stringstream, bool> > m_actionCode;
	std::unordered_map<std::string, std::string> m_generatedCode;

	int m_tab = 0;
	std::stringstream m_createCode;
	std::stringstream m_generatedFunctionsCode;
	std::stringstream m_output;
	std::stringstream m_parseActionFunction;
	...
	std::stringstream& output() { return *m_currentStream; }
	...
	std::string getLevelTarget() const;
	...
	void Visit(struct gs1::StmtWith* node) override;
...
public:
	...
	bool testCodeForErrors(const std::string& code, std::string* errorOutput);
