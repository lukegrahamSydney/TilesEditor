...
{
    QRegularExpression expr("\\b" + QRegularExpression::escape(cursor.selectedText()) + "\\b");
    auto searchIterator = cursor;
    searchIterator.movePosition(QTextCursor::Start);

    
    searchIterator = document()->find(expr, searchIterator);
...
void QCodeEditor::keyPressEvent(QKeyEvent* e) {
    ...
    const int defaultIndent = 4;// tabStopDistance() / fontMetrics().averageCharWidth();
#else
          ...
          e->modifiers() == Qt::NoModifier)
      {
          ...
          if (cursor.hasSelection())
          {
            ...
            for (auto line = startLine; line <= endLine; ++line)
            {
                ...
                cursor.insertText(QString(defaultIndent, ' '));
    ...
    if (m_replaceTab && e->key() == Qt::Key_Backtab)
    {
      ...
      for (auto line = startLine; line <= endLine; ++line)
      {
          ...
          for (int i = 0; i < 4; ++i)
          {
              ...
              if (this->document()->characterAt(position) == ' ')
              {
              ...
              else if (this->document()->characterAt(position) == '\t' && i == 0)
              {
