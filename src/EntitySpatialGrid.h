...
namespace TilesEditor
{
    ...
    public:
        ...
        int search(const QRectF& rect, bool accurate, QList<T*>& output, bool (*f)(T*, void* userData), void* userData)
        {
            ...
            int right = (int)std::min(std::ceil((x + rect.width()) / m_cellWidth), (double)m_hcount);
            int bottom = (int)std::min(std::ceil((y + rect.height()) / m_cellHeight), (double)m_vcount);
        ...
        int search(const QRectF& rect, bool accurate, QSet<T*>& output, bool (*f)(T*, void* userData), void* userData)
        {
            static uint64_t currentSearchIndex = 0;
        ...
        T* searchFirst(const QRectF& rect, bool accurate, bool (*f)(T*, void* userData), void* userData)
        {
            auto x = rect.x() - m_x;
            auto y = rect.y() - m_y;
        ...
        T* entityAt(const QPointF& point)
        {
            auto x = point.x() - m_x;
            auto y = point.y() - m_y;
        ...
        void add(T* entity)
        {
            ...
            auto x = boundingBox.x() - m_x;
            auto y = boundingBox.y() - m_y;
            ...
            entity->m_spacialGridRight = (int)std::min(std::ceil((x + boundingBox.width()) / m_cellWidth), (double)m_hcount);
            entity->m_spacialGridBottom = (int)std::min(std::ceil((y + boundingBox.height()) / m_cellHeight), (double)m_vcount);
        ...
        void updateEntity(T* entity)
        {
            ...
            int left = (int)std::max(std::floor(x / m_cellWidth), 0.0);
            int top = (int)std::max(std::floor(y / m_cellHeight), 0.0);
            int right = (int)std::min(std::ceil((x + boundingBox.width()) / m_cellWidth), (double)m_hcount);
            int bottom = (int)std::min(std::ceil((y + boundingBox.height()) / m_cellHeight), (double)m_vcount);
