...
namespace TilesEditor
{
	...
	void LevelLink::setProperty(const QString& name, const QVariant& value)
	{
		AbstractLevelEntity::setProperty(name, value);

		if (name == "nextLevel")
			setNextLevel(value.toString());

		else if (name == "nextX")
			setNextX(value.toString());

		else if (name == "nextY")
			setNextY(value.toString());

		else if (name == "nextLayer")
			setNextLayer(value.toInt());
	}
	...
	void LevelLink::draw(QPainter* painter, const QRectF& viewRect, double x, double y)
	{
