...
namespace TilesEditor
{
    ...
    bool LevelFormatGraal::saveLevel(Level* level, QIODevice* stream)
    {
        ...
        if (tilemap)
        {
            ...
            QByteArray baddiesBytes;
            QByteArray npcsBytes;
            QByteArray chestsBytes;

            QBuffer baddiesStream(&baddiesBytes);
            QBuffer npcsStream(&npcsBytes);
            QBuffer chestsStream(&chestsBytes);

            baddiesStream.open(QIODeviceBase::WriteOnly);
            npcsStream.open(QIODeviceBase::WriteOnly);
            chestsStream.open(QIODeviceBase::WriteOnly);
            for (auto obj : sortedObjects)
            {
                switch (obj->getEntityType())
                {
                    case LevelEntityType::ENTITY_BADDY:
                    {
                        ...
                        baddiesStream.write(&x, 1);
                        baddiesStream.write(&y, 1);
                        baddiesStream.write(&type, 1);

                        auto verses = QString("%1\\%2\\%3\\").arg(baddy->getBaddyVerse(0)).arg(baddy->getBaddyVerse(1)).arg(baddy->getBaddyVerse(2));
                        baddiesStream.write(verses.toLocal8Bit());
                        baddiesStream.write("\n");
                    }
                    break;
                    ...
                    case LevelEntityType::ENTITY_CHEST:
                    {
                        ...
                        if (itemIndex >= 0)
                        {
                            ...
                            chestLine.append('\n');
                            chestsStream.write(chestLine.data());
                        }
                    }
                    break;
